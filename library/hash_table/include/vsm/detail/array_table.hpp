...
struct _array_table_bucket
{
	...
	[[nodiscard]] vsm_always_inline _array_table_bucket load() const
	{
		return *this;
	}
	...
	vsm_no_sanitize_address
	[[nodiscard]] vsm_always_inline I load_index() const
	{
		return index;
	}
	...
	vsm_no_sanitize_address
	[[nodiscard]] vsm_always_inline _array_table_hash_t<I> load_hash() const
	{
		return hash;
	}
	...
	vsm_no_sanitize_address
	vsm_always_inline void swap(_array_table_bucket& other) noexcept
	{
		{
...
template<typename I>
_array_table_bucket<I> const _array_table_empty_bucket =
{
	...
	, _array_table_hash_mask<I>
{
	void _set(size_t const hash_mask, bool const is_local)
	{
		...
		this->m_hash_mask = static_cast<I>(hash_mask);
...
struct _array_table_policies
{
	...
	explicit _array_table_policies(any_cvref_of<P> auto&& policies)
...
struct _array_table_allocator
{
	...
	explicit _array_table_allocator(any_cvref_of<A> auto&& allocator)
...
constexpr size_t _array_table_storage_size(size_t const hash_mask, size_t const sizeof_t)
{
	return
		...
		_array_table_max_size(hash_mask) * sizeof_t;
}
...
{
	_array_table_update_annotation<SizeofT>(
		table,
		table.m_size,
		_array_table_max_size(table.m_hash_mask));
}
#endif
...
void _array_table_resize_1(_array_table_with_allocator<I, P, A>& table, size_t new_hash_mask)
{
	...
	for (size_t i = 0; i < old_bucket_count; ++i)
	{
		...
		{
			_array_table_insert_1(new_buckets, new_hash_mask, old_bucket.load());
		}
	}
...
{
	_array_table_resize_1<T, K>(table, static_cast<size_t>(table.m_hash_mask) * 2 + 1);
}
...
public:
	...
	[[nodiscard]] iterator find(Key const& key)
	{
		...
		return storage == nullptr
			? end()
			: iterator(static_cast<T*>(storage));
	}
	...
	[[nodiscard]] const_single_iterator find(Key const& key) const
	{
		...
		return storage == nullptr
			? end()
			: const_single_iterator(static_cast<T const*>(storage));
	}
	...
	// TODO: Erase by key.
	void erase(const_single_iterator iterator);
	...
	[[nodiscard]] iterator end()
	{
		...
		return reinterpret_cast<T*>(data) + this->m_size;
	}
