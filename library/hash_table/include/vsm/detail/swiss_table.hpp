...
private:
	...
	static vsm_always_inline Integer load(_swiss_table_ctrl const* const ctrl) noexcept
	{
		Integer integer;
	...
	vsm_no_sanitize_address
	static vsm_always_inline void store(_swiss_table_ctrl* const ctrl, Integer const integer) noexcept
	{
...
public:
	...
	iterator_type match(_swiss_table_ctrl const hash_2) const
	{
	...
	{
		__m128i const v_end = _mm_set1_epi8(static_cast<int8_t>(_swiss_table_ctrl::end));
		uint32_t const mask = static_cast<uint32_t>(_mm_movemask_epi8(_mm_cmpgt_epi8(v_end, m_ctrl)));
...
private:
	vsm_no_sanitize_address
	static vsm_always_inline __m128i load(_swiss_table_ctrl const* const ctrl) noexcept
	{
		return _mm_loadu_si128(reinterpret_cast<__m128i const*>(ctrl));
	}

	vsm_no_sanitize_address
	static void vsm_always_inline store(_swiss_table_ctrl* const ctrl, __m128i const v_ctrl) noexcept
	{
...
struct _swiss_table_policies
{
	...
	explicit _swiss_table_policies(auto&& policies)
...
struct _swiss_table_allocator
{
	...
	explicit _swiss_table_allocator(auto&& allocator)
