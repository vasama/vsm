...
public:
	...
	explicit arena_resource(std::span<std::byte> const storage) noexcept
		: m_pos(storage.data())
		, m_end(storage.data() + storage.size())
	{
	}

	allocation allocate(size_t const min_size, size_t const max_size) noexcept
	{
		if (static_cast<size_t>(m_end - m_pos) < min_size)
		{
	...
	position_type get_position() const noexcept
	{
...
static_assert(monotonic_memory_resource<arena_resource>);

TEST_CASE("any_monotonic_allocator", "[any][allocator]")
{
	std::byte arena_storage[256];
	arena_resource arena(arena_storage);
