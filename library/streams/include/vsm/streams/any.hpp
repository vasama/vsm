...
struct any_sink_direct_write_release
{
	...
	template<direct_sink Sink>
	static vsm::result<void> invoke(Sink& sink, size_t const size) noexcept
	{
...
public:
	template<sink Sink>
	constexpr any_sink_ref(Sink&& sink)
		: any_ref(vsm_forward(sink))
	{
	}
...
public:
	template<direct_sink Sink>
	constexpr any_sink_direct_ref(Sink&& sink)
		: any_ref(vsm_forward(sink))
	{
	}
	...
	[[nodiscard]] constexpr vsm::result<void> direct_write_release(size_t const size) const noexcept
	{
		return any_ref::invoke<detail::any_sink_direct_write_release>(size);
	}
};
