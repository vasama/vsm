...
private:
	...
	static constexpr size_t max_style_characters = 3 + 4 * 4 + 2 * 5;

	static size_t format_to_buffer(style style, char* buffer);

	friend result<size_t> tag_invoke(
		streams::print_t const cpo,
		streams::sink auto& sink,
		style const style)
	{
