...
#include <vsm/streams/format.hpp>
...
public:
	...
	swiss_map<std::string_view, option_view> m_long_options;
	...
	result<void> process_completed()
	{
		if (any_flags(m_flags, flags::require_command) && m_command == nullptr)
		{
			...
			return vsm::unexpected(error::command_not_given);
		}
...
group& app::group(std::string_view const name)
{
	vsm_self(private_class);
	return self->create_group(self->m_resource, name);
}

option& app::option(std::string_view const name)
{
	vsm_self(private_class);
	return self->create_option(self->m_resource, name, nullptr, false);
}

option& app::flag(std::string_view const name)
{
	vsm_self(private_class);
...
{
	return print_help(streams::cout);
}

result<void> app::print_help_to_stderr() const
{
	return print_help(streams::cerr);
}

result<void> app::print_help(streams::any_sink_ref const sink) const
{
	vsm_self(private_class);
...
group_internal& app_internal::create_group(resource& resource, std::string_view const name)
{
	...
	return *self->m_groups.push_back(
		group_internal::create(this, resource, name));
}

option_internal& app_internal::create_option(
	resource& resource,
	...
	bool const flag)
{
	...
	{
		auto const spec_part = std::string_view(spec_part_range);
		...
		case 0:
			{
				if (flag)
				{
					self->report_configuration_error("Flags may not be positional.");
				}

				if (spec_part.data() != spec.data())
				{
					self->report_configuration_error(
						"Positional forms of options must be defined before non-positional forms.");
				}
		...
		case 1:
			{
				...
				{
					self->report_configuration_error(
						"Short option name must be a single character ({})",
						form);
				}

				if (!self->m_short_options.insert(form.front(), view).inserted)
				{
					self->report_configuration_error("Duplicate option: '{}'", form);
				}
		...
		case 2:
			{
				if (!self->m_long_options.insert(form, view).inserted)
				{
					self->report_configuration_error("Duplicate option: '{}", form);
				}
...
result<size_t> app_impl::_print_help(streams::any_sink_ref const sink) const
{
	...
	{
		return vsm::discard_value(add_size(r));
	};
	...
	auto const print_section_header = [&](std::string_view const header) -> result<void>
	{
		...
		vsm_try_void(use_size(format(sink, "{}:\n", header)));
		return {};
	};
	...
	auto const print_user_message = [&](std::string_view const message) -> result<void>
	{
		...
		{
			vsm_try_void(print_section_separator());
			vsm_try_void(use_size(streams::print(sink, message)));
	...
	auto const print_help = [&](size_t const width, std::string_view const help) -> result<void>
	{
		...
		if (!help.empty())
		{
			...
			for (auto const& line_range : help | std::views::split('\n'))
			{
	...
	auto const print_option_help = [&](
		option_internal const& option,
		std::string_view const name) -> result<void>
	{
		vsm_try(width, add_size(format(sink, "  {} ", name)));

		if (!option.m_help_parameter.empty())
		{
			vsm_try(part_width, add_size(format(sink, "{} ", option.m_help_parameter)));
			width += part_width;
		}

		if (option.m_min > 0)
		{
			vsm_try(part_width, add_size(streams::print(sink, "REQUIRED ")));
			width += part_width;
		}
	...
	// Usage.
	if (vsm::any_flags(m_flags, flags::show_usage))
	{
		...
		{
			auto const print_usage_part = [&](
				std::string_view const string,
				bool const required) -> result<void>
			{
				if (required)
				{
					return use_size(streams::format(sink, " {}", string));
				}
				else
				{
					return use_size(streams::format(sink, " [{}]", string));
			...
			static_assert(std::ranges::range<decltype((m_positional_options))>);
			for (option_internal const* const option : m_positional_options)
			{
				if (is_visible(*option))
				{
					vsm_try_void(print_usage_part(option->m_help_positional, option->m_min > 0));
				}
			}

			if (m_visible_command_count > 0)
			{
				vsm_try_void(print_usage_part(
					"COMMAND",
					vsm::any_flags(m_flags, flags::require_command)));
			}
	...
	if (m_visible_command_count > 0)
	{
		...
		for (auto const& [name, command] : m_commands)
		{
			...
			{
				vsm_try(width, add_size(streams::format(sink, "  {} ", command->m_name)));
