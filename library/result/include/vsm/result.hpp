...
[[nodiscard]] inline std::error_code make_error_code(std::errc const e)
{
...
template<bool HasValueType>
struct _try_value_type;

template<>
struct _try_value_type</* HasValueType: */ false>
{
...
template<>
struct _try_value_type</* HasValueType: */ true>
{
	template<typename T, typename DefaultType>
...
template<typename T>
concept has_try_value_type = requires { typename T::value_type; };

template<typename T, typename DefaultType>
using try_value_type = _try_value_type<has_try_value_type<T>>::template type<T, DefaultType>;

} // namespace detail

#define vsm_detail_try_value_t(result, default) \
	::vsm::detail::try_value_type<::std::remove_cvref_t<decltype(result)>, default>

#define vsm_detail_try_is_void(result) \
	(::std::is_void_v<vsm_detail_try_value_t(result, void)>)
...
#define vsm_detail_try_is_ptr(result) \
	(::std::indirectly_readable<vsm_detail_try_value_t(result, int*)>)
