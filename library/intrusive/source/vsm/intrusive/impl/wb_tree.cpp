...
static_assert(check_incomplete_tagged_ptr<ptr<hook>>());
...
static void rebalance(hook** const root, hook** node, bool l, bool const insert)
{
	...
	while (node != root)
	{
		...
		if (parent->weight > 2 && weight(l_child) >= weight(r_child) * delta)
		{
			...
			// TODO: Solve this using an assertion.
...
void _wb::erase(hook* const node)
{
	...
	if (l_child != nullptr || r_child != nullptr)
	{
		...
		// It is known that the heavier side of the tree cannot be null.
		vsm_assert((succ_l ? r_child : l_child) != nullptr);
