...
class access
{
#define vsm_detail_intrusive_link_t \
	decltype(detail::match_link<Tag, MinSize>(std::declval<T const&>()))

#if __INTELLISENSE__
	template<typename T, typename Tag, size_t MinSize>
	struct _link_t
	{
		// Intellisense requires the indirection to avoid an internal compiler error.
		using type = vsm_detail_intrusive_link_t;
	};

	template<typename T, typename Tag, size_t MinSize = 0>
	using link_t = typename _link_t<T, Tag, MinSize>::type;
#else
	template<typename T, typename Tag, size_t MinSize = 0>
	using link_t = vsm_detail_intrusive_link_t;
#endif

#undef vsm_detail_intrusive_link_t
