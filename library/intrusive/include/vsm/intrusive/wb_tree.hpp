...
struct _wb
{
	...
	[[nodiscard]] hook* select(size_t rank) const;
	[[nodiscard]] size_t rank(hook const* node) const;
	...
	template<typename T, typename Tag>
	[[nodiscard]] static hook** get_iterator_ptr(iterator<T, Tag> const& it)
	{
		return it.m_children;
	}

	[[nodiscard]] static hook** iterator_begin(hook** root);
	[[nodiscard]] static hook** iterator_advance(hook** children, bool l);
};
