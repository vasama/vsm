...
{
	template<typename T>
	using ptr = incomplete_tagged_ptr<T, bool, 1>;
	...
	template<typename T, typename Tag>
	[[nodiscard]] static ptr<hook>* get_iterator_ptr(iterator<T, Tag> const& it)
	{
		return it.m_children;
	}

	[[nodiscard]] static ptr<hook>* iterator_begin(ptr<hook>* root);
	[[nodiscard]] static ptr<hook>* iterator_advance(ptr<hook>* children, bool l);
};
...
private:
	...
	[[nodiscard]] ptr<hook> const* _lower_bound(Key const& key) const
	{
	...
	template<typename Key>
	[[nodiscard]] find_result _find(Key const& key) const
	{
