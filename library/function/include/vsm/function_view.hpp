...
template<>
struct is_invocable_using_impl</* Noexcept: */ false>
{
...
template<>
struct is_invocable_using_impl</* Noexcept: */ true>
{
	template<typename T, typename R, typename... Ps>
...
public:
	...
	template<typename F>
		requires
			...
			&& std::is_function_v<F>
	/*constexpr*/ function_view_impl(F& function)
	...
	template<typename F>
		requires
			is_invocable_using<F*, N, R, Ps...>
			&& std::is_function_v<F>
	/*constexpr*/ function_view_impl(F* const function)
		: m_invoke(invoke_function<N, F, R, Ps...>)
		, m_context{ .function = function }
	{
	}
	...
	/*constexpr*/ R operator()(Args&&... args) const noexcept(N)
	{
