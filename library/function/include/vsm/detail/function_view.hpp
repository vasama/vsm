...
{
	return const_cast<T&>(*static_cast<T const*>(context.object))(vsm_forward(args)...);
}

template<bool N, typename F, typename R, typename... Ps>
/*constexpr*/ R invoke_function(function_context const context, Ps... args) noexcept(N)
{
	return static_cast<F*>(context.function)(vsm_forward(args)...);
}

template<bool N, auto F, typename R, typename... Ps>
/*constexpr*/ R invoke_nontype_unused(function_context, Ps... args) noexcept(N)
{
	return F(vsm_forward(args)...);
}
...
{
	return F(const_cast<T&>(*static_cast<T const*>(context.object)), vsm_forward(args)...);
}
