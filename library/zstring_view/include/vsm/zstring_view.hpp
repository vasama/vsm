...
public:
	...
	explicit constexpr basic_zstring_view(Char const* const c_str, size_t const size)
		: m_view(c_str, size)
	{
		vsm_assert(traits_type::length(c_str) == size);
	}
	...
	{
		return m_view.starts_with(c_str);
	}
...
struct std::hash<vsm::basic_zstring_view<Char>> : std::hash<std::basic_string_view<Char>>
{
};
