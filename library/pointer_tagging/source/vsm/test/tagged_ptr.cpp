...
using incomplete_ptr_1 = incomplete_tagged_ptr<s<1>, bool, true>;
using incomplete_ptr_2 = incomplete_tagged_ptr<s<2>, bool, true>;
...
static_assert(!check_incomplete_tagged_ptr<incomplete_ptr_1>());
static_assert(check_incomplete_tagged_ptr<incomplete_ptr_2>());


template<typename T>
using ptr = tagged_ptr<T, bool>;
...
TEST_CASE("tagged_ptr", "[pointer_tagging][tagged_ptr]")
{
	s<2> object = { 42 };
...
TEST_CASE("tagged_ptr static_pointer_cast", "[pointer_tagging][tagged_ptr]")
{
	using b = s<2>;
...
TEST_CASE("tagged_ptr const_pointer_cast", "[pointer_tagging][tagged_ptr]")
{
	using t = s<2>;
...
TEST_CASE("tagged_ptr reinterpret_pointer_cast pointer", "[pointer_tagging][tagged_ptr]")
{
	using t = s<2>;
	using t_ptr = ptr<t>;
...
TEST_CASE("tagged_ptr reinterpret_pointer_cast integer", "[pointer_tagging][tagged_ptr]")
{
	using t = s<2>;
	using t_ptr = ptr<t>;
...
TEST_CASE("tagged_ptr dynamic_pointer_cast", "[pointer_tagging][tagged_ptr]")
{
	struct b
	{
		virtual ~b() = default;
