...
struct _vector_allocator
{
	...
	explicit _vector_allocator(any_cvref_of<A> auto&& allocator)
...
#	define vsm_vector_annotate(function, ...) \
		((void)sizeof((__VA_ARGS__))) /* NOLINT */
...
T* _vector_resize(_vector_base<A>& vector, size_t const new_size)
{
	...
	else if (vsm_likely(new_size < old_size)) // NOLINT(readability-misleading-indentation)
	{
...
void _vector_swap(_vector_base<A>& lhs, _vector_base<A>& rhs)
{
	if constexpr (MayBeLocal)
	{
		if (lhs.m_is_local && rhs.m_is_local)
		{
			...
			else if (rhs_ptr != rhs_end) // NOLINT(readability-misleading-indentation)
			{
				vsm_vector_annotate(
					modify,
					rhs,
...
public:
	...
	[[nodiscard]] T* release()
	{
		...
		T* const ptr = this->template _get_storage_ptr<T>();
	...
	void adopt(T* ptr, size_t size, size_t capacity) noexcept;
