...
struct _any_new_base_constraint_1
{
	template<bool D, size_t C, typename... OtherFunctions>
	static void match(_any_new_base<D, C, Functions..., OtherFunctions...> const&);
};

template<typename T, typename... Functions>
concept _any_new_base_constraint = requires
{
	_any_new_base_constraint_1<Functions...>::match(std::declval<T const&>());
};
...
{
	function_ptr_t const* m_functions;
