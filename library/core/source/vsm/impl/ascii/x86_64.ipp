static __m128i _ascii_transform_k(
	__m128i const vsrc,
	__m128i const vsub,
	__m128i const vadd,
	__m128i const vmax)
...
static void _ascii_transform_n(
...
{
	__m128i const vsub = _mm_set1_epi8(static_cast<char>(sub));
	__m128i const vadd = _mm_set1_epi8(static_cast<char>(add));
	__m128i const vmax = _mm_set1_epi8(static_cast<char>(max));
	...
	for (; index < limit; index += 16)
	{
		...
		__m128i const vout = _ascii_transform_k(vsrc, vsub, vadd, vmax);
		_mm_storeu_si128(reinterpret_cast<__m128i*>(out_beg + index), vout);
	}

	for (; index < size; ++index)
	{
		auto const src_value = static_cast<unsigned char>(src_beg[index]);
		auto const out_value = _ascii_transform_1(src_value, sub, add, max);
		out_beg[index] = static_cast<char>(out_value);
	}
}

static int _ascii_compare(
	...
	size_t const size)
{
	...
	for (; index < limit; index += 16)
	{
		...
		__m128i const vlhs_2 = _ascii_transform_k(vlhs_1, vsub, vadd, vmax);
		__m128i const vrhs_2 = _ascii_transform_k(vrhs_1, vsub, vadd, vmax);
		...
		if (mask != 0xFFFF)
		{
			size_t const mismatch = index + static_cast<size_t>(std::countl_one(mask));
	...
	for (; index < size; ++index)
	{
		...
		auto const lhs_2 = _ascii_transform_1(lhs_1, sub, add, max);
		auto const rhs_2 = _ascii_transform_1(rhs_1, sub, add, max);
