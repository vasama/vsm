...
public:
	...
	[[nodiscard]] vsm_detail_atomic_inline T load(
	...
	[[nodiscard]] vsm_detail_atomic_inline T exchange(
		T const value,
		[[maybe_unused]] std::memory_order const memory_order) const noexcept
	{
		T& object = *m_object;
	...
	[[nodiscard]] vsm_detail_atomic_inline bool compare_exchange_weak(
		T& expected,
		T const desired,
	...
	[[nodiscard]] vsm_detail_atomic_inline bool compare_exchange_strong(
		T& expected,
		T const desired,
		[[maybe_unused]] std::memory_order const success,
		[[maybe_unused]] std::memory_order const failure) const noexcept
