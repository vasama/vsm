...
vsm_always_inline constexpr bool vsm_detail_assert_bool(T const& value)
	requires requires (T const& t) { t ? 0 : 1; }
{
...
#define vsm_detail_assert_fail(...) \
	(vsm_assert_fail(__FILE__, __LINE__, #__VA_ARGS__) ? vsm_debugbreak() : (void)0)

#if vsm_static_analyzer
#	define vsm_detail_assert_1 vsm_analysis_assume

#	define vsm_detail_assert_0 vsm_detail_assert_1
#	define vsm_detail_verify_0 vsm_detail_assert_1
#else
#	define vsm_detail_assert_1(...) \
		(vsm_detail_assert_bool(__VA_ARGS__) ? (void)0 : vsm_detail_assert_fail(__VA_ARGS__))

#	define vsm_detail_assert_0(...) ((void)0)
#	define vsm_detail_verify_0(...) ((void)(__VA_ARGS__))
#endif
...
#else
#	define vsm_assert(...) vsm_detail_assert_0(__VA_ARGS__)
#	define vsm_verify(...) vsm_detail_verify_0(__VA_ARGS__)
#	define vsm_assume(...) vsm_optimize_assume(__VA_ARGS__)
#endif
...
#else
#	define vsm_assert_slow(...) vsm_detail_assert_0(__VA_ARGS__)
#	define vsm_verify_slow(...) vsm_detail_verify_0(__VA_ARGS__)
#	define vsm_assume_slow(...) vsm_optimize_assume(__VA_ARGS__)
#endif
