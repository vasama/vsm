...
param(
	...
	[switch]$ShowAnalysisCommand=$false
)
...
if (!$Step -or $Step -eq 'conan-install') {
	...
	Invoke-NativeCommand conan install '-pr:a' "$PSScriptRoot/conan/profiles/$ConanProfile" @ConanArguments .
}
...
if ((!$Step -and $Analysis -ne $null) -or $Step -eq 'analyze') {
	...
	foreach ($Result in $Results) {
		if ($Result.ExitCode -ne 0) {
			if ($ShowAnalysisCommand) {
				$Result.CompileCommand.command
			}
			$Result.Output
			$Status = $False
		}
