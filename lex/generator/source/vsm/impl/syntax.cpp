...
struct lex_parser : parser
{
	...
	recursion<expr*> parse_expr(precedence const precedence)
	{
		...
		case tk::lbrace:
			{
				auto expr_node = node<brace_expr>(sk::min_max_expr);
	...
	rule* parse_rule()
	{
		...
		{
			auto attr_list = list<attr>();
	
			while (peek_token()->kind == tk::at)
			{
				auto attr_node = node<attr>();
				attr_node->at_token = consume_token();
				attr_node->name_token = consume_token(tk::identifier);
				attr_list.push(attr_node.leave());
			}

			rule_node->attr = attr_list.leave();
		}
