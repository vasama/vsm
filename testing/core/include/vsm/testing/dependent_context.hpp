...
struct dependent_context
{
	...
	dependent_context(auto lambda) noexcept
	{
...
template<>
struct dependent_context::select</* Bool: */ false>
{
	template<typename T>
	static T identity(T);
};
...
template<>
struct dependent_context::select</* Bool: */ true>
{
...
#define vsm_dependent_context \
	...
	vsm_detail_dependent_cat(vsm_detail_dependent_context, vsm_detail_dependent_counter) = \
		[]<bool vsm_detail_dependent_context = true>() noexcept -> auto
