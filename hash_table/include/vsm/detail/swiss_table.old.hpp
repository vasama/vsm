...
struct iterator_core
{
	...
	{
		iterator_core it;
		it.m_ctrl = get_ctrls(slots, element_size, capacity);
		it.m_slot = slots;
...
public:
	...
	[[nodiscard]] friend bool operator==(basic_iterator const& self, sentinel)
	{
		return *self.m_ctrl == ctrl_end;
	}
};
...
public:
	...
	using sentinel                                          = swiss_table::sentinel;
	...
	{
		return iterator(iterator_core::begin<sizeof(T)>(m.slots, m.capacity));
	}

	[[nodiscard]] const_iterator begin() const
	{
		return const_iterator(iterator_core::begin<sizeof(T)>(m.slots, m.capacity));
	}
